...
macro_rules! headers {
    (
        $(
            $(#[$docs:meta])*
            ($name:ident, $name_bytes:literal);
        )+
    ) => {
        $(
            $(#[$docs])*
            pub const $name: Bytes = Bytes::from_static($name_bytes);
        )+
    }
}
...
headers! {
    ...
    (UPGRADE_INSECURE_REQUESTS, b"upgrade-insecure-requests");
    ...
    (X_DNS_PREFETCH_CONTROL, b"x-dns-prefetch-control");
    ...
    (X_FORWARDED_FOR, b"x-forwarded-for");
    ...
    (X_FORWARDED_HOST, b"x-forwarded-host");
    ...
    (X_FORWARDED_PROTO, b"x-forwarded-proto");
